...
class SIMON():
    def __init__(self,WordSize=16,Alpha=8,Beta=1,Gamma=2):
    ...
    def SearchDiffLinear(self, Round,RoundLinear, TargetWeight, SolverName="cadical153", AllTrailsObtained=[],
                         HWInDIffL=[],HWInDIffR=[],InDiff=[],MatSuiBound=True):
            ...
            MinBoundLimits={}
            if MatSuiBound:
                LinearBound = self.SIMONLinearBound[str(self.WordSize * 2)]
                if LinearBound != []:
                    for r in range(RoundLinear):
                        if LinearBound[r]!=0:
            ...
            if HWInDIffL!=[]:
                ...
                AllVars = self.SequentialAddition(InDiffL, HWInDIffL[0], HWInDIffL[1])

            if HWInDIffR!=[]:
                InDiffR = self.NameToID(self.Diff[0])[self.WordSize:]
                AllVars = self.SequentialAddition(InDiffR, HWInDIffR[0], HWInDIffR[0])
...
if __name__ == "__main__":
    ...
    if SearchDiff:
        ...
        NumTrails = 3
        TrunctedDiff = False
        SpecfiedDiff=[0,0x00000001]

        # ******************************************************
        #
        #                   Search process
        SampleSIMON = SIMON(WordSize=WordSize)
        SampleSIMON.SearchDiffStepByStep(Round,TrunctedDiff=TrunctedDiff,NumTrails= NumTrails,StartWeight=0,SpecfiedDiff=SpecfiedDiff,SolverName="cadical153")
    ...
    if SearchLinear:
        ...
        #                   Search process
        SampleSIMON = SIMON(WordSize=WordSize)
        SampleSIMON.SearchLinearStepByStep(Round,NumTrails)
    ...
    if  SearchDl:
        ...
        #limit the Hamming weight of the input differene
        HWInDIffL = []
        HWInDIffR = []
        ...
        NAME="SIMON"+str(WordSize*2)+"_Conf_"+str(Round+RoundLinear)+"(" +str(Round)+","+ str(RoundLinear)+")-"

        if (HWInDIffL!=[]):
            NAME+="HWInDiffL-("+str(HWInDIffL[0])+","+str(HWInDIffL[1])+")"
        if (HWInDIffR != []):
            NAME += "HWInDiffL-(" + str(HWInDIffR[0]) + "," + str(HWInDIffR[1]) + ")"
        ...
        #                   Search process
        SampleSIMON=SIMON(WordSize=WordSize)
        #SampleSIMON.SearchDiffStepByStep(Round,TrunctedDiff=False,NumTrails= NumTrails,StartWeight=0,SolverName="cadical153")
        #SampleSIMON.SearchLinearStepByStep(Round,NumTrails)

        #SampleSIMON.SearchDiffLinearStepByStep( Round, RoundLinear, NumTrails=NumTrails,)
        SampleSIMON.SearchDiffLinearTop( Round, RoundLinear,ScopeRound=2 ,NumTrails=NumTrails,StartWeight=StartWeight,
